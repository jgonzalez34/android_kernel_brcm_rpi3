...
#define BME_OVERSAMPLING_T_MAX    BME_VAL_NAME(OVERSAMP_16X)
/*! define maximum pressure oversampling */
#define BME_OVERSAMPLING_P_MAX    BME_VAL_NAME(OVERSAMP_16X)
/*! define maximum humidity oversampling */
#define BME_OVERSAMPLING_H_MAX    BME_VAL_NAME(OVERSAMP_16X)
/*! define defalut filter coefficient */
#define BME_FILTER_DEFAULT        BME_VAL_NAME(FILTER_COEFF_8)
/*! define maximum filter coefficient */
#define BME_FILTER_MAX            BME_VAL_NAME(FILTER_COEFF_16)
...
static int bme_check_chip_id(struct bme_data_bus *data_bus)
{
	...
	err = data_bus->bops->bus_read(BME_I2C_ADDRESS,
		BME_REG_NAME(CHIP_ID_REG), &chip_id, 1);
	if (err < 0) {
		err = -EIO;
		...
		int32_t *temperature)
{
	...
	err = BME_CALL_API(read_uncomp_temperature)(&utemperature);
	if (err)
		return err;
	*temperature = BME_CALL_API(compensate_temperature_int32)(utemperature);
	return err;
}
...
static int bme_get_pressure(struct bme_client_data *data, uint32_t *pressure)
{
	...
	err = BME_CALL_API(read_uncomp_pressure)(&upressure);
	if (err)
		return err;

	*pressure = (BME_CALL_API(compensate_pressure_int64)(upressure))>>8;
	return err;
}
...
static int bme_get_humidity(struct bme_client_data *data, uint32_t *humidity)
{
	...
	err = BME_CALL_API(read_uncomp_humidity)(&uhumidity);
	if (err)
		return err;
	/*check the excel of user API, essentially it has to shift 10 bits*/
	*humidity = (BME_CALL_API(compensate_humidity_int32)(uhumidity));
	return err;
}
...
{
	BME_CALL_API(get_oversamp_temperature)(&data->oversampling_t);
		...
		uint8_t oversampling)
{
	...
	err = BME_CALL_API(set_oversamp_temperature)(oversampling);
	if (err)
		return err;
...
{
	BME_CALL_API(get_oversamp_pressure)(&data->oversampling_p);
		...
		uint8_t oversampling)
{
	...
	err = BME_CALL_API(set_oversamp_pressure)(oversampling);
	if (err)
		return err;
...
{
	BME_CALL_API(get_oversamp_humidity)(&data->oversampling_h);
		...
		uint8_t oversampling)
{
	...
	err = BME_CALL_API(set_oversamp_humidity)(oversampling);
	if (err)
		return err;
...
{
	BME_CALL_API(get_power_mode)(&data->op_mode);
...
static int bme_set_op_mode(struct bme_client_data *data, uint8_t op_mode)
{
	...
	err = BME_CALL_API(set_power_mode)(op_mode);
	if (err)
		return err;
 ...
 * @retval 4000 4000ms
*/
static uint32_t bme_get_standby_durn(struct bme_client_data *data)
{
	uint8_t standbydur;

	BME_CALL_API(get_standby_durn)(&standbydur);
 ...
 * @retval non-zero failed
*/
static int bme_set_standby_durn(struct bme_client_data *data,
		uint32_t standbydur)
{
	int err = 0;
	int i;
	...
	err = BME_CALL_API(set_standby_durn)(i);
	if (err)
		return err;
			...
			const char *buf, size_t count)
{
	...
	if (status == 0) {
		mutex_lock(&data->lock);
		bme_set_standby_durn(data, standbydur);
		mutex_unlock(&data->lock);
		return count;
	}
	return status;
...
static int bme_init_client(struct bme_client_data *data)
{
	...
	status = bme_set_standby_durn(data, BME_STANDBYDUR_DEFAULT);
	if (status)
		return status;
